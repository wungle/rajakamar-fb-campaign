...
class FacebookHelper extends AppHelper {
	...
	*/
	...
	// Share button
	function share_me(refferal, caption) {
		// Check if sdk loaded
		checkIfLoaded();
		...
		var publish = {
			...
			description: 'KLIK DISINI Untuk Ikutan! Caranya Gampang Banget! 3 Orang Pemenang Akan berlibur GRATIS bersama Rajakamar ke Singapore! Syarat & Ketentuan berlaku.'
		};
	...
	function share_refferal(refferal, caption) {
		...
		FB.api('/me/feed', 'POST', publish, function(response) {  
			if (response && response.id) {
		        location.href = '" . Router::url('/', true) . "campaignUsers/share_score/" . $campaign . "';
			}
		});
	};
