 ...
 */
	public function admin_view($id = null) {
		if (!$id) {
			$this->Session->setFlash(__d('users', 'Invalid User.'), 'flash_error');
	...
	public function admin_add() {
		if (!empty($this->request->data)) {
			...
			if ($this->{$this->modelClass}->add($this->request->data)) {
				$this->Session->setFlash(__d('users', 'The User has been saved'), 'flash_success');
				$this->redirect(array('action' => 'index'));
			}
		}
 ...
 */
	public function admin_edit($userId = null) {
		if($this->Auth->user('id') != $userId) {
			$this->Session->setFlash(__d('users', "Sorry, you don't have access."), 'flash_error');
 ...
 */
	public function admin_delete($userId = null) {
		...
		if ($this->{$this->modelClass}->delete($userId)) {
			$this->Session->setFlash(__d('users', 'User deleted'), 'flash_success');
		} else {
			$this->Session->setFlash(__d('users', 'Invalid User'), 'flash_error');
		}
 ...
 */
	public function add() {
		if ($this->Auth->user()) {
			$this->Session->setFlash(__d('users', 'You are already registered and logged in!'), 'flash_error');
			$this->redirect('/');
		}
		...
		if (!empty($this->request->data)) {
			...
			if ($user !== false) {
				...
				$this->Session->setFlash(__d('users', 'Your account has been created. You should receive an e-mail shortly to authenticate your account. Once validated you will be able to login.'), 'flash_success');
				$this->redirect(array('action' => 'login'));
			} else {
				unset($this->request->data[$this->modelClass]['password']);
				unset($this->request->data[$this->modelClass]['temppassword']);
				$this->Session->setFlash(__d('users', 'Your account could not be created. Please, try again.'), 'flash_error');
			}
		}
	}
		...
		if ($this->request->is('post')) {
			if ($this->Auth->login()) {
				...
				if (empty($data[$this->modelClass]['return_to'])) {
					$data[$this->modelClass]['return_to'] = '/admin/dashboards';
				}

				$this->redirect($this->Auth->redirect($data[$this->modelClass]['return_to']));
			} else {
				$this->Session->setFlash('Invalid e-mail / password combination.  Please try again', 'flash_error');
			}
		}
 ...
 */
	public function logout() {
		...
		$this->RememberMe->destroyCookie();
		$this->Session->setFlash(sprintf(__d('users', '%s you have successfully logged out'), $user[$this->{$this->modelClass}->displayField]), 'flash_success');
		...
		if ($this->request->is('post')) {
			try {
				if ($this->{$this->modelClass}->checkEmailVerification($this->request->data)) {
					$this->_sendVerificationEmail($this->{$this->modelClass}->data);
					$this->Session->setFlash(__d('users', 'The email was resent. Please check your inbox.'), 'flash_success');
					$this->redirect('login');
				} else {
					$this->Session->setFlash(__d('users', 'The email could not be sent. Please check errors.'), 'flash_error');
				}
			} catch (Exception $e) {
				$this->Session->setFlash($e->getMessage());
			}
 ...
 */
	public function verify($type = 'email', $token = null) {
		...
		try {
			...
			$this->Session->setFlash(__d('users', 'Your e-mail has been validated!'), 'flash_success');
 ...
 */
	public function request_new_password($token = null) {
		...
		if (!$data) {
			$this->Session->setFlash(__d('users', 'The url you accessed is not longer valid'), 'flash_error');
			return $this->redirect('/');
		}
		...
		if ($this->{$this->modelClass}->save($data, array('validate' => false))) {
			...
			$this->Session->setFlash(__d('users', 'Your password was sent to your registered email account'), 'flash_success');
			return $this->redirect(array('action' => 'login'));
		}

		$this->Session->setFlash(__d('users', 'There was an error verifying your account. Please check the email you were sent, and retry the verification link.'), 'flash_error');
		$this->redirect('/');
	}
	...
	public function admin_change_password() {
		if ($this->request->is('post')) {
			...
			if ($this->{$this->modelClass}->changePassword($this->request->data)) {
				$this->Session->setFlash(__d('users', 'Password changed.'), 'flash_success');
			...
			if (!empty($user)) {
				...
				if ($admin) {
					$this->Session->setFlash(sprintf(
						...
						$user[$this->modelClass]['email']), 'flash_success');
					$this->redirect(array('action' => 'index', 'admin' => true));
				} else {
					$this->Session->setFlash(__d('users', 'You should receive an email with further instructions shortly'), 'flash_success');
					$this->redirect(array('action' => 'login'));
				}
			} else {
				$this->Session->setFlash(__d('users', 'No user was found with that email.'), 'flash_error');
 ...
 */
	protected function _resetPassword($token) {
		...
		if (empty($user)) {
			$this->Session->setFlash(__d('users', 'Invalid password reset token, try again.'), 'flash_error');
			$this->redirect(array('action' => 'reset_password'));
		}

		if (!empty($this->request->data) && $this->{$this->modelClass}->resetPassword(Set::merge($user, $this->request->data))) {
			$this->Session->setFlash(__d('users', 'Password changed, you can now login with your new password.'), 'flash_success');
